<template>
  <div>
    ...
    <el-row style="margin:16px 0" :gutter="12">
      ...
      <el-col :span="3"><el-select v-model="category" placeholder="分类"><el-option v-for="c in cats" :key="c" :label="c" :value="c" /></el-select></el-col>
...
const filtered = computed(() => items.value.filter(i =>
  (!search.value || i.name.includes(search.value)) &&
