...
function executeTool(name, args) {
  switch (name) {
    ...
    case 'get_weather': {
      const mock = {
        北京: { temp: 25, cond: '晴', icon: '☀️' },
        上海: { temp: 28, cond: '多云', icon: '⛅' },
        重庆: { temp: 23, cond: '暴雨', icon: '🌧️' },
        广州: { temp: 30, cond: '雷阵雨', icon: '⛈️' },
      }
    ...
    case 'get_user_profile':
      return {
        content: [
          { type: 'text', text: '用户画像：风格偏好(简约通勤、法式浪漫)，肤色(冷白皮)，脸型(鹅蛋脸)，BMI 21.0' },
        ],
      }
