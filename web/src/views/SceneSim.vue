...
import SceneWeather from './scene/SceneWeather.vue'
...
<template>
  <div class="page">
    <PageHeader title="情景模拟" to="/home">
      <template #right>
        <button class="diary-btn" @click="showToast('穿搭日记开发中 📔')">📔 穿搭日记</button>
      </template>
    </PageHeader>
    ...
    <div class="body scroll-y hide-scrollbar">
      ...
      <template v-if="tab === 'try'">
        <section>
          <h2 class="sec-title">穿搭适用场景选择</h2>
        ...
        <section>
          ...
          <div class="preview-grid">
            ...
            >
              ...
              <div class="pv-acts" @click.stop>
                <button class="pv-icon" :class="{ liked: favScenes.has(s.key) }" @click="toggleFav(s.key)">
                  {{ favScenes.has(s.key) ? '❤️' : '🤍' }}
                </button>
                <button class="pv-icon" @click="saveLook">⭐</button>
              </div>
            </div>
          </div>
          <button class="btn btn-primary save-btn" @click="saveLook">保存这套造型</button>
        </section>
      </template>
      ...
      <template v-else>
        <section>
          <div class="reco-head">
            <h2 class="sec-title">AI 搭配方式推荐</h2>
            <button class="refresh" :class="{ spin: aiLoading }" aria-label="换一套" @click="onRefresh">🔄</button>
          </div>

          <!-- 状态 -->
          <p v-if="aiLoading" class="status load">🤖 AI 正在按天气 + 你的风格生成搭配…</p>
          <p v-else-if="usingAi" class="status ok">✨ 由 AI 实时生成（点 🔄 换一套）</p>
          <p v-else-if="aiError" class="status err">⚠️ {{ aiError }}（先看本地示意，点 🔄 重试）</p>

          <p class="reco-sub">{{ shownReco.title }}</p>
          <p v-if="shownReco.reason" class="reco-reason">{{ shownReco.reason }}</p>

          <div class="reco-grid">
            <div v-for="(p, i) in shownReco.pieces" :key="i" class="reco-item">
        ...
        <div class="cta">
          <button class="btn cta-ghost" @click="buyAll">🛍️ 一键购买</button>
          <button class="btn btn-primary cta-primary" @click="matchAll">🧥 一键搭配</button>
        </div>
      </template>

      <!-- 底部：穿搭搭配方式选择 -->
      <section class="modes">
        <h2 class="sec-title modes-title">穿搭搭配方式选择</h2>
