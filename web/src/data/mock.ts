/*
 * 全站 mock 数据。
 * 每个实体都带一个 img 路径（指向 public/images/... 下的真实图片）。
 * 图片放进 public/images 后自动显示；没有对应文件时，TileImage 会自动回退到
 * 渐变色块 + emoji 占位，所以缺图也不影响运行。
 * 需要放哪些图、叫什么名字，见项目根目录 IMAGE-MANIFEST.md。
 */
...
export const OUTFIT_RECOS: OutfitReco[] = OUTFIT_RECOS_RAW.map((r) => ({
  ...r,
  pieces: r.pieces.map((p, i) => ({ ...p, img: `/images/outfit/${r.id}-${i + 1}.png` })),
}))
...
export const AI_FEATURES: {
  ...
  { key: 'create', label: '个性化创建', desc: '打造你的专属虚拟形象', emoji: '🧍‍♀️', from: '#ffd6e8', to: '#d6a0ff', route: '/create' },
  { key: 'stylist', label: 'AI 穿搭顾问', desc: '在线问穿搭，实时回答', emoji: '🤖', from: '#c9f0e6', to: '#7fd0c0', route: '/stylist' },
  { key: 'free', label: '自由搭配', desc: '给形象自由换装试穿', emoji: '🧥', from: '#c9d8ff', to: '#9ab0ff', route: '/free-match' },
  { key: 'scene', label: '情景模拟', desc: '看天气 + AI 一键搭配', emoji: '🌦️', from: '#c9ecff', to: '#8fc9f0', route: '/scene' },
  { key: 'renew', label: '旧衣新生', desc: '旧衣服焕发新搭法', emoji: '♻️', from: '#d6f0d9', to: '#9fceb0', route: '/scene' },
  { key: 'magazine', label: '时尚杂志', desc: '灵感 & 穿搭思路', emoji: '📖', from: '#f3e0d6', to: '#d8b08f', route: '/magazine' },
  { key: 'community', label: '时尚社群', desc: '和同好交流穿搭', emoji: '💬', from: '#efe0f0', to: '#c29ad6', route: '/community' },
]
